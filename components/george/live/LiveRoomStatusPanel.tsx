...
function ControlPill({
  ...
  const toneClass =
    tone === "conversation"
      ? "bg-[#337FCB]/[0.10] hover:bg-[#3D91E6]/[0.16]"
      : tone === "support"
        ? "bg-[#286BB4]/[0.075] hover:bg-[#3A82CF]/[0.14]"
        : "bg-[#245E9B]/[0.065] hover:bg-[#3479C3]/[0.13]";
  ...
  return (
    <button
      ...
      title={`${label}: ${detail}`}
      className={`group flex min-h-[82px] min-w-0 overflow-hidden items-center justify-between gap-4 rounded-[1.05rem] px-4 py-3.5 text-left ring-1 ring-inset ring-[#82BFFF]/[0.035] transition-[transform,background-color,opacity] duration-200 active:scale-[0.985] disabled:cursor-not-allowed disabled:opacity-40 sm:min-h-[66px] sm:rounded-[0.95rem] sm:px-3.5 sm:py-2.5 ${
        active ? "bg-[#3B8CDE]/[0.20]" : toneClass
      }`}
    >
      <span className="min-w-0">
        <span className="block text-[18px] font-semibold leading-6 text-white/88 sm:text-[15px] sm:leading-5">
          {label}
        </span>
        <span className="mt-1 block truncate text-[13px] leading-5 text-white/42 sm:mt-0.5 sm:text-[11px] sm:leading-4">
          {detail}
        </span>
      </span>
      <span
        className={`h-1.5 w-1.5 shrink-0 rounded-full transition-[background-color,box-shadow] ${
          active
            ? "bg-[#78BFFF] shadow-[0_0_10px_rgba(120,191,255,0.48)]"
            : "bg-[#9CCBFF]/24"
        }`}
      />
    </button>
...
export function LiveRoomStatusPanel({
  ...
  const controlGrid = (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-3">
      ...
      <ControlPill
        label="Reading"
        detail="Open reading surface"
        active={readingIntroComplete && viewMode === "controls"}
        tone="conversation"
        disabled={isThinking}
      ...
      <div className="sm:hidden">
        {viewMode === "controls" ? (
          <div className="flex min-h-[calc(100dvh-190px)] flex-col">
            ...
            <div className="rounded-[1.35rem] bg-[radial-gradient(circle_at_top,rgba(46,126,215,0.07),rgba(5,7,11,0.92)_52%)] p-2.5 shadow-[0_20px_64px_rgba(0,0,0,0.42)] ring-1 ring-inset ring-[#7CB7FF]/[0.04] backdrop-blur-xl">
              {controlGrid}
