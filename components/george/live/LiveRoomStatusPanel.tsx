    ...
    <div className={`pointer-events-auto w-full max-w-[430px] md:max-w-[520px] md:max-w-[780px] xl:max-w-[980px] md:max-w-[720px] xl:max-w-[860px] md:max-w-[720px] xl:max-w-[860px] rounded-[1.15rem] border px-4 py-3 transition duration-300 ${liveRoomActive ? 'border-white/[0.055] bg-[#05070B]/82 shadow-[0_22px_80px_rgba(0,0,0,0.46),inset_0_1px_0_rgba(255,255,255,0.035)]' : 'border-white/[0.035] bg-[#05070B]/58 opacity-72 shadow-[0_14px_48px_rgba(0,0,0,0.32)]'}`}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${isListening ? 'bg-[#8FB6C9] shadow-[0_0_14px_rgba(143,182,201,0.65)]' : 'bg-[#D7DBE4]/22'}`} />
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-[9px] md:text-[10px] leading-4">
        <div className={`rounded-[0.8rem] border px-3 py-2 ${isListening ? 'border-[#8FB6C9]/[0.18] bg-[#8FB6C9]/[0.07] text-[#8FB6C9]' : 'border-white/[0.045] bg-white/[0.018] text-[#D7DBE4]/42'}`}>
        ...
        <button
          ...
          disabled={isThinking}
          className={`rounded-[0.8rem] border px-3 py-2 text-left transition active:scale-[0.985] disabled:cursor-not-allowed disabled:opacity-40 ${liveRoomActive ? 'border-[#8FB6C9]/[0.18] bg-[#8FB6C9]/[0.055] text-[#DCEBFF]/72' : 'border-white/[0.045] bg-white/[0.018] text-[#D7DBE4]/42 hover:border-[#8FB6C9]/[0.18] hover:bg-[#8FB6C9]/[0.045]'}`}
        >
          <span className="block uppercase tracking-[0.16em] text-[#BFD9FF]/34">LIVE</span>
          <span className="mt-1 flex items-center justify-between gap-2">
            <span>{liveRoomActive ? 'ON' : 'OFF'}</span>
            <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full border text-[10px] ${liveRoomActive ? 'border-[#8FB6C9]/24 bg-[#8FB6C9]/18 text-[#8FB6C9]' : 'border-white/[0.08] bg-white/[0.025] text-white/34'}`}>
              {liveRoomActive ? '●' : '○'}
            </span>
          </span>
        </button>
        <button
          ...
          className={`rounded-[0.8rem] border px-3 py-2 text-left transition active:scale-[0.985] ${voiceOn ? 'border-[#8FB6C9]/[0.16] bg-[#8FB6C9]/[0.055] text-[#D7DCFF]/72' : 'border-white/[0.045] bg-white/[0.018] text-[#D7DBE4]/42 hover:border-[#8FB6C9]/[0.16] hover:bg-[#8FB6C9]/[0.045]'}`}
        >
          <span className="block uppercase tracking-[0.16em] text-[#BFD9FF]/34">AUDIO</span>
          <span className="mt-1 flex items-center justify-between gap-2">
            <span>{voiceOn ? 'ON' : 'OFF'}</span>
            <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full border text-[10px] ${voiceOn ? 'border-[#8FB6C9]/24 bg-[#8FB6C9]/18 text-[#D7DCFF]' : 'border-white/[0.08] bg-white/[0.025] text-white/34'}`}>
              {voiceOn ? '●' : '○'}
            </span>
          </span>
        </button>
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-[9px] md:text-[10px] leading-4">
        <button
          ...
          disabled={isThinking}
          className={`rounded-[0.95rem] border px-4 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.025)] transition duration-150 active:scale-[0.985] active:border-[#8FB6C9]/[0.28] active:bg-[#8FB6C9]/[0.105] hover:border-[#8FB6C9]/[0.22] hover:bg-[#8FB6C9]/[0.085] disabled:cursor-not-allowed disabled:opacity-40 ${liveRoomActive ? 'border-[#8FB6C9]/[0.20] bg-[#8FB6C9]/[0.075] text-[#DCEBFF]/72 shadow-[0_0_26px_rgba(143,182,201,0.055),inset_0_1px_0_rgba(255,255,255,0.035)]' : 'border-[#8FB6C9]/[0.10] bg-[#8FB6C9]/[0.026] text-[#DCEBFF]/36'}`}
        >
          <span className="block uppercase tracking-[0.16em] text-[#BFD9FF]/34">Receiver</span>
          <span className="mt-1 flex items-center justify-between gap-2">
            <TypewriterLabel value={receiverProfileLabel} />
            <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full border text-[12px] transition ${liveRoomActive ? 'border-[#8FB6C9]/24 bg-[#8FB6C9]/18 text-[#8FB6C9]' : 'border-white/[0.06] bg-white/[0.025] text-white/30'}`}>
              {liveRoomActive ? '◉' : '○'}
            </span>
          </span>
        </button>
