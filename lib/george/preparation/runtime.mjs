...
export function prepareConversationFromPackage(input = {}) {
  ...
  const summaries = [
    ...normalizeList(input.summaries || pkg.liveSummaries || pkg.conversationSummaries),
    record.summary ? { summary: record.summary, source: 'conversation_record' } : null,
  ].filter(Boolean)

  const learning = [
    ...normalizeList(input.learning || pkg.learning),
  ...
  const packageForPreparation = {
    ...
    conversationContext: clean(pkg.conversationContext) || clean(record.conversationContext),
  }
