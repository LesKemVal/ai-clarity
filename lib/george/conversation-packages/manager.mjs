...
export function createConversationPackage(input = {}, options = {}) {
  ...
  const pkg = {
    ...
    relevantDocumentation: normalizeList(input.relevantDocumentation),
  ...
  const next = {
    ...
    relevantDocumentation:
      updates.relevantDocumentation === undefined
        ...
        : normalizeList(updates.relevantDocumentation),
    conversations:
...
export function attachDocumentation(pkg, documentation, options = {}) {
  ...
  const relevantDocumentation = uniqueByKey(
    ...
    (item) => typeof item === 'string' ? item : item.id || item.name || item.title
  )
