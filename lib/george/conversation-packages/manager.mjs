...
function normalizeFormulaSelection(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null
  }

  const formulaId = String(value.formulaId || '').trim()
  const formulaVersion = Number(value.formulaVersion)
  const source =
    value.source === 'user'
      ? 'user'
      : value.source === 'george'
  ...
  if (
    !formulaId ||
    !Number.isFinite(formulaVersion) ||
    formulaVersion < 1 ||
    !source
  ) {
    return null
  ...
  return {
    formulaId,
    formulaVersion: Math.floor(formulaVersion),
    source,
  }
}
...
export function createConversationPackage(input = {}, options = {}) {
  ...
  const pkg = {
    ...
    formulaSelection: normalizeFormulaSelection(input.formulaSelection),
...
export function updateConversationPackage(pkg, updates = {}, options = {}) {
  const next = {
    ...
    ...updates,
    formulaSelection:
      updates.formulaSelection === undefined
        ? normalizeFormulaSelection(pkg.formulaSelection)
        : normalizeFormulaSelection(updates.formulaSelection),
