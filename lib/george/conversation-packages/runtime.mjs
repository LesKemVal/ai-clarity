...
function outcomeProgressionFromReview(review = {}) {
  ...
  const hasEvidence =
    String(review.desiredOutcome || review.desiredState || '').trim() ||
    String(review.currentState || '').trim() ||
    String(review.observedChange || review.notes || '').trim() ||
    String(review.bestAvailablePath || '').trim() ||
    normalizeList(review.availablePaths).length > 0 ||
...
function opportunityContinuityFromLiveResult(liveResult = {}) {
  ...
  return {
    ...
    opportunityHealth: opportunityContinuity.opportunityHealth || null,
    outcomeEffect: opportunityContinuity.outcomeEffect || 'unknown',
    outcomeConsistency: opportunityContinuity.outcomeConsistency || null,
