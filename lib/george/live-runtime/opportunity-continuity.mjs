import { buildOutcomeConsistency } from './outcome-consistency.mjs'
...
function boundConfidence(value) {
  return Math.max(0, Math.min(95, Math.round(value)))
}

function opportunityHealthLevel(strong, usable, weak) {
...
export function buildOpportunityContinuity(input = {}) {
  ...
  let opportunityState = 'unknown'
  let executionDecision = 'gather_missing_evidence'
  let timing = 'wait'
