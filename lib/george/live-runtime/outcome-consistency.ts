export type OutcomeConsistencyState =
  | 'compatible'
  | 'tension'
  | 'contradiction'
...
export type OutcomeConsistencyInput = {
  desiredOutcome?: string | null
  secondaryOutcome?: string | null
  possibleSecondaryOutcome?: string | null
  objectiveEvolution?: string | null
  roomSignal?: string | null
}

export type OutcomeConsistencyDecision = {
  primaryOutcome: string
  secondaryOutcome: string
  consistency: OutcomeConsistencyState
  preserveBothViable: boolean
  userAuthorityRequired: boolean
...
export function buildOutcomeConsistency(input: OutcomeConsistencyInput = {}): OutcomeConsistencyDecision {
  ...
  const contradictionDetected = evidenceTension
  const tension = pressureTension
  const contradiction = impossible
    ...
    : contradictionDetected
      ...
      : tension
        ? 'The secondary outcome may require restraint while pursuing the primary desired outcome.'
        : ''

  const consistency: OutcomeConsistencyState =
    impossible ? 'impossible_combination' : contradictionDetected ? 'contradiction' : tension ? 'tension' : 'compatible'
