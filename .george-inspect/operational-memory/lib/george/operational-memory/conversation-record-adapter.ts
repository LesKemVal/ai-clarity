...
function buildSignals(
  ...
  const highlightSignals = highlights.map<OperationalSignal>(
    (highlight, index) => ({
  ...
  const hypothesisSignals = hypotheses.map<OperationalSignal>(
    (hypothesis, index) => ({
      id: `hypothesis-${index + 1}`,
      type: normalizeType(
        hypothesis.type || hypothesis.hypothesis,
...
function buildOutcomes(
  ...
  const observedProgress = normalizeType(
    latestOutcome.observedProgress,
    'unknown'
  )
  ...
  return [
    {
      ...
      achieved:
        observedProgress === 'advanced' ||
        observedProgress === 'achieved',
...
export function adaptConversationRecordForOperationalMemory(
  ...
  const startedAt =
    input.startedAt ??
    parseTimestamp(input.record.createdAt, endedAt)

  const transcriptHighlights = input.record.transcriptHighlights || []
  const behaviorHypotheses = input.record.behaviorHypotheses || []
  ...
  return {
    id:
      input.record.id ||
      `operational-${input.record.packageId || endedAt}`,
    userId: input.userId,
